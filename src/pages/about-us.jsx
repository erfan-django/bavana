    ...
    return (
        <div>
            <div className={`h-[80px] sm:h-[90px] lg:h-[100px] bg-[#ED8F44] rounded-[50px] flex items-center cursor-pointer`} onClick={() => setIsOpen(!isOpen)}>
                <span className="rounded-full dark:bg-[#251D16] bg-white w-[60px] sm:w-[70px] lg:w-[80px] h-[60px] sm:h-[70px] lg:h-[80px] flex items-center justify-center mr-3 ml-4">
                    <Arrowdown className={`transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`} />
                </span>
                <p className="text-white dark:text-[#251D16] text-sm sm:text-base lg:text-lg mx-2 sm:mx-3 lg:mx-4">{faq.question}</p>
            </div>
            {isOpen && (
                <div className="mx-4 sm:mx-5 lg:mx-7 pr-4 sm:pr-5 lg:pr-6 bg-[#F4A261] rounded-[50px] flex items-center p-2 sm:p-3 lg:p-4 mt-2 transition-all duration-300" style={{ overflow: "hidden" }}>
                    <p className="text-white dark:text-[#251D16] text-xs sm:text-sm lg:text-base mx-2 sm:mx-3 lg:mx-4">{faq.answer}</p>
                </div>
            )}
        </div>
    )
    ...
    return (
        <div>
            <div className="flex flex-col lg:flex-row p-4 justify-center lg:justify-evenly items-center">
                <div>
                    <h1 className="text-lg sm:text-xl lg:text-4xl/relaxed lg:w-[467px] font-bold text-[#8D5215] dark:text-[#DF7F1D] mb-4 sm:mb-6 lg:mb-6 pt-6 sm:pt-8 lg:pt-9 mx-2 sm:mx-2 lg:mx-2 text-justify">درباره باوانا ما اینجا هستیم تا تجربه ارام ، متمرکز و شخصی سازی شده برای مطالعه بسازیم .</h1>
            ...
            <div className="flex flex-col gap-4 sm:gap-5 lg:gap-6 p-2 sm:p-4 lg:p-0">
                <h2 className="text-[#8D5215] text-xl sm:text-2xl lg:text-4xl font-semibold mb-4 sm:mb-6 lg:mb-[28px] dark:text-[#DF7F1D]">سوالات متداول</h2>
                {faqs.map((faq, index) => (
